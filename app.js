...
let issues=[],editing=null,blocked=false,editRevision=0,personal=null;
...
function openEditor(i){editRevision=Classroom.revision;editing=i?.id??null;$('#form').reset();$('#error').textContent='';$('#editor-title').textContent=i?'이슈 수정':'이슈 등록';if(i)for(const k of ['title','owner','due','priority','status'])$('#form').elements[k].value=i[k];$('#editor').showModal();$('#form').elements.title.focus();}
async function remove(i){const rev=Classroom.revision;if(confirm(`“${i.title}” 이슈를 삭제하시겠습니까?`)){try{const next=issues.filter(x=>x.id!==i.id);if(Classroom.active){await Classroom.save(next,rev);}else{issues=next;persist();render();}}catch(error){notify(error.message);}}}
$('#add').onclick=()=>openEditor();$('#cancel').onclick=()=>$('#editor').close();$('#filter').onchange=render;
$('#form').onsubmit=async e=>{e.preventDefault();const f=$('#form').elements;const item={id:editing??crypto.randomUUID(),title:f.title.value.trim(),owner:f.owner.value.trim(),due:f.due.value,priority:f.priority.value,status:f.status.value};const error=core.validate(item);if(error){$('#error').textContent=error;if(!item.title)f.title.focus();return;}try{if(editing&&!issues.some(i=>i.id===editing))throw Error('이 항목이 삭제되었습니다. 새로 등록해 주세요.');const next=editing?issues.map(i=>i.id===editing?item:i):[...issues,item];if(Classroom.active){await Classroom.save(next,editRevision);}else{issues=next;persist();render();}$('#editor').close();}catch(error){$('#error').textContent=error.message;}};
...
Classroom.attach({board:'issues',validate(items){return core.decode(JSON.stringify({version:1,issues:items}));},enter(){personal=issues;issues=[];$('#editor').close();render();},receive(items){issues=items;render();},leave(){issues=personal;personal=null;$('#editor').close();render();notify('개인 모드 · 기존 브라우저 데이터를 표시합니다.');}});
